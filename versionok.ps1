...
$global:cancelHandler = $null

# Gestionnaire d'événements pour Ctrl+C (méthode compatible)
try {
    $null = [Console]::TreatControlCAsInput = $false
    
    # Créer un delegate pour gérer Ctrl+C
    $global:cancelHandler = {
        param($sender, $e)
        Write-Host ""
        Write-Host "🛑 Ctrl+C détecté - Arrêt en cours..." -ForegroundColor Yellow
    ...
    Write-Host "⚠️  Impossible de configurer Ctrl+C handler: $($_.Exception.Message)" -ForegroundColor Yellow
    Write-Host "   Le script fonctionnera mais Ctrl+C pourrait ne pas fonctionner parfaitement." -ForegroundColor Yellow
}
...
try {
    ...
    while ($listener.IsListening -and -not $global:shouldStop) {
        try {
            # Vérifier s'il y a une requête en attente (avec timeout)
            if ($listener.IsListening -and $listener.GetContextAsync().Wait(1000)) {
    ...
    try {
        if ($global:cancelHandler) {
            [Console]::remove_CancelKeyPress($global:cancelHandler)
        }
    } catch {
